...
function Room() {
      ...
      >
        ...
        <div>
          <div className="text-xl font-medium text-black">
            Chat: {chat.id} <br /> with user {chat.user2ID}
          </div>
        </div>
      </button>
    );
  ...
  function CreateNewChat(otheruserid) {
    Axios.get("https://localhost:7081/api/Chat/" + userid + otheruserid).then(
      (response) => {
  ...
  return (
    <div>
      ...
      <div className="overflow-y-auto h-fill absolute left-0">{ChatList}</div>
